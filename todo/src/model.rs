...
impl Task {
    pub fn all(conn: &MysqlConnection) -> QueryResult<Vec<Task>> {
        all_tasks.order(tasks::id.desc()).load::<Task>(conn)
    }

    pub fn insert(todo: NewTask, conn: &MysqlConnection) -> QueryResult<usize> {
        diesel::insert_into(tasks::table)
            .values(&todo)
            .execute(conn)
    }

    pub fn toggle_with_id(id: i32, conn: &MysqlConnection) -> QueryResult<usize> {
